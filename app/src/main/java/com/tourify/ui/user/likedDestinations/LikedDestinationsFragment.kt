...
@AndroidEntryPoint
class LikedDestinationsFragment : Fragment() {
    private val viewModel: LikedDestinationsViewModel by viewModels()

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?): View {
        return inflater.inflate(R.layout.fragment_liked_destinations, container, false)
    }
    ...
    private fun loadLikedDestinations() {
        viewModel.likedDestinationsResponse.observe(viewLifecycleOwner) {
            ...
            if (destinations != null) {
                val layout = view?.findViewById<ViewGroup>(R.id.liked_destinations_grid_layout)
                updateSection(destinations, layout!!)
            }
        }

        viewModel.getLikedDestinations(object: CoroutinesErrorHandler {
    ...
    private fun setupImageClickListener(imageView: ImageView, id : Int) {
        imageView.setOnClickListener {
            ...
            findNavController().navigate(R.id.action_navigation_likedDestinations_to_navigation_destination, args)
        }
    }
}